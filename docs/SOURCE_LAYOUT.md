...
| **`Networking/`** | DHCP + Harmony on `SetIP`, flow/`AddAppPerformance` gate, reachability (L3 enforce), game subnet/customer caches, private LAN registry, IPv4 helpers, router vs switch classification. |
...
- **Networking:** `DHCPManager.cs`, `ReachabilityService.cs`, `GameSubnetHelper.cs`, `CustomerPrivateSubnetRegistry.cs`, `Ipv4Rfc1918.cs`, `RouteMath.cs`, `NetworkDeviceClassifier.cs`
